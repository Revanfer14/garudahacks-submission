...
export const AudioPlayer = ({ content }: AudioPlayerProps) => {
  ...
  const handlePlayAudio = async () => {
    ...
    if (serverError) {
      toast.error(
        "Server TTS tidak tersedia. Pastikan backend berjalan di port 5000."
      );
      return;
    }
    ...
    try {
      setIsLoading(true);
      const voiceName =
        voices.find((v) => v.id === selectedVoice)?.name || "Suara default";
      ...
      if (error instanceof Error) {
        if (error.message.includes("fetch")) {
          setServerError(true);
          toast.error(
            "Tidak dapat terhubung ke server TTS. Pastikan backend berjalan."
          );
        } else {
